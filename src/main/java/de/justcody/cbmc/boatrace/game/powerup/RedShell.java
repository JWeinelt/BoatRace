...
public class RedShell extends PowerUp {
    private UUID target;

    public RedShell(UUID thrower) {
        super(thrower);
        type = Game.PowerUpItem.RED_SHELL;
    }

    public RedShell setTarget(UUID uuid) {
        target = uuid;
        return this;
    }
    ...
    public Vector calculateVelocity() {
        ...
        vel.add(new Vector(0,0,0));
        if(height > 3 && loc.getY() > targetLoc.getY()) {
